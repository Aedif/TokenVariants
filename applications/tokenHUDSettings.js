export default class TokenHUDSettings extends FormApplication {
  ...
  async getData(options) {
    ...
    data.tokenHUDWildcardActive =
      game.modules.get('token-hud-wildcard')?.active;
    const settings = game.settings.get('token-variants', 'hudSettings');
    settings.enableWorldSettings =
      game.user &&
      game.user.can('FILES_BROWSE') &&
      game.user.can('TOKEN_CONFIGURE');
    if (settings.enableWorldSettings) {
      settings.enableSideMenuForAll = game.settings.get(
        'token-variants',
        'enableTokenHUDButtonForAll'
      );
      settings.displaySharedOnly = game.settings.get(
        'token-variants',
        'displayOnlySharedImages'
      );
      settings.disableSideMenuIfTHW = game.settings.get(
        'token-variants',
